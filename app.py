...
if st.session_state.user_capital > 0 and st.session_state.user_units > 0:
    ...
    with col2:
        ...
        st.markdown(f"<div class='metric-card'><sup>Current Nifty 50 Price</sup><br><h4>₹{round(nifty_current, 2)}</h4></div>", unsafe_allow_html=True)
    ...
    st.info("👋 **Welcome to the P.A.S.E. Allocation Terminal.**\n\nSlide open the left parameters menu (`»`) and input your total invested capital along with your current mutual fund units. The system matrix will instantly compute your personalized tracking metrics and live execution signals!")
